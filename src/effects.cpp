...
void acid_effect (void *vo, int level, int dam, int target)
{
	...
	if (obj->carried_by)
	{
	   ...
	   if ((IS_OBJ_STAT(obj, ITEM_NOPURGE)
	       || IS_OBJ_STAT(obj, ITEM_NOBREAK))
		   || ((!IS_NPC(victim) && IS_SET(victim->act, PLR_QUESTING))
		       || (!IS_NPC(victim) && IS_SET(victim->act, PLR_TOURNEY))))
				return;
	}
        ...
        {                        /* etch it */
            ...
            if (obj->condition <= 0) 
                if (obj->carried_by != NULL)
                {
                  ...
                  act("$p breaks into pieces.", obj->carried_by, obj, obj, TO_CHAR);
                }
...
void fire_effect (void *vo, int level, int dam, int target)
{
    ...
    {                            /* do the effect on a victim */
        ...
        if (!IS_AFFECTED (victim, AFF_BLIND)
            && !saves_spell (level / 4 + dam / 20, victim, DAM_FIRE) && number_bits(4) == 0)
        {
            AFFECT_DATA af;
