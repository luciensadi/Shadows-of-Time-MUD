...
case SECT_FOREST:	send_to_char("`2@",ch);		break;
case SECT_FIELD:	send_to_char("`@.",ch);		break;
case SECT_HILLS:	send_to_char("`3~",ch);		break;
case SECT_ROAD:		send_to_char("`8#",ch);		break;
case SECT_MOUNTAIN:	send_to_char("`3^",ch);		break;
case SECT_WATER_SWIM:	send_to_char("`$:",ch);		break;
case SECT_WATER_NOSWIM:	send_to_char("`4:",ch);		break;
case SECT_UNUSED:	send_to_char("`3X",ch);		break;
case SECT_AIR:		send_to_char("`^%",ch);		break;
case SECT_DESERT:	send_to_char("`#=",ch);		break;
case SECT_ENTER:	send_to_char("`8@",ch);		break;
case SECT_INSIDE:	send_to_char("`%%",ch);		break;
case SECT_CITY:		send_to_char("`^X",ch);		break;
case SECT_ROCK_MOUNTAIN:send_to_char("`8^",ch);		break;
case SECT_SNOW_MOUNTAIN:send_to_char("`&^",ch);		break;
case SECT_SWAMP:	send_to_char("`6&",ch);		break;
case SECT_JUNGLE:	send_to_char("`2&",ch);		break;
case SECT_RUINS:	send_to_char("`%#",ch);		break;
case SECT_VOLCANIC_MOUNTAIN: send_to_char("`1^",ch);    break;
case SECT_WALL:		send_to_char("`&#`7",ch);		break;
case SECT_GATE:		send_to_char("`#=`7",ch);	break;
case SECT_JUNCTION:	send_to_char("`7+",ch);		break;
case SECT_EDGE:		send_to_char("`7-",ch);		break;
case SECT_VEDGE:         send_to_char("`7|",ch);         break;
case SECT_TOWER:        send_to_char("`7^",ch);		break;
case SECT_TAKEN:	send_to_char("`8G`7",ch);	break;
case SECT_EMPTY:          send_to_char(" ",ch);           break;
case (SECT_MAX+1):	send_to_char("`3?",ch);		break;
...
case SECT_RUINS:        send_to_char("#",ch);           break;
case SECT_VOLCANIC_MOUNTAIN: send_to_char("^",ch);       break;
case SECT_WALL:         send_to_char("#",ch);           break;
case SECT_GATE:         send_to_char("=",ch);           break;
case SECT_JUNCTION:     send_to_char("+",ch);           break;
case SECT_EDGE:         send_to_char("-",ch);           break;
case SECT_TOWER:        send_to_char("^",ch);         break;
case SECT_VEDGE:         send_to_char("|",ch);         break;
case SECT_TAKEN:          send_to_char("G",ch);           break;
case SECT_EMPTY:          send_to_char(" ",ch);           break;
...
case SECT_DESERT:       send_to_char("`#=",ch);         break;
case SECT_ENTER:        send_to_char("`8@",ch);         break;
case SECT_INSIDE:       send_to_char("`1%",ch);         break;
case SECT_CITY:         send_to_char("`^X",ch);         break;
case SECT_ROCK_MOUNTAIN:send_to_char("`8^",ch);         break;
case SECT_SNOW_MOUNTAIN:send_to_char("`&^",ch);         break;
case SECT_SWAMP:        send_to_char("`6&",ch);         break;
...
case SECT_CITY:		fprintf(fp,"X");		break;
...
map[min-1][min-1]=SECT_MAX;
map[max][max]=SECT_MAX; /* mlk :: rounds edges */
map[min][max]=SECT_MAX;
map[max][min]=SECT_MAX;
...
if (x==min) {
	sprintf(buf,"`7   `$%s`7",ch->in_room->name);
	send_to_char(buf,ch);


/* mlk :: no brief in wilderness, ascii map is automatic, autoexits off too
        if (  !IS_NPC(ch) && IS_SET(ch->act, PLR_AUTOEXIT)  ) { 
        send_to_char( " `2",ch);do_exits( ch, "auto" );send_to_char( "`7",ch);
        } */
...
if (!IS_IMMORTAL(ch))
{
  size=URANGE(ch->mapsize,size,MAX_MAP);
}
else
{
  size=URANGE(ch->mapsize,size,MAX_MAP);
...
map[center][center]=SECT_MAX+2; /* can be any number above SECT_MAX+1 	*/
 ...
 if (ch->in_room->sector_type == SECT_TAKEN || ch->in_room->sector_type == SECT_TOWER || !IS_SET(ch->act2, PLR_WORLDMAP))
 {
 ...
 if (ch->in_room->sector_type == SECT_EMPTY)
 {
      ch->in_room->name = "Unowned Territory";
      ch->in_room->description = "This territory is waiting to be claimed.";
...
if (IS_IMMORTAL(ch) && IS_SET(ch->act2, PLR_WORLDMAP)) {
   ShowMap (ch, min, max); 
   return;
   }
*/
...
void do_mapsize( CHAR_DATA *ch, char *argument )
{
  ...
  if (value > 50)
  {
    send_to_char("Map size is too large, please retry.", ch);
    return;
  }
  else if (value < 1)
  {
    send_to_char("Map size is too small, please retry.", ch);
    return;
  }
  else
  {
    sprintf(buf, "Map size changed to: %d", value);
    send_to_char(buf, ch);
    ch->mapsize = value;
    return;
  }
...
void do_showthemap( CHAR_DATA *ch, char *argument )
{
int size,center,x,y,min,max;
char arg1[10];
...
case SECT_VOLCANIC_MOUNTAIN:
strcpy(name,"On a Volcanic Mountain");
strcpy(desc,"You are on a volcanic mountain near Shayol Ghul. \
The rock juts from the ground like a broken bone from skin, \
and is just as jagged and dangerous. \
");
  if (exitsum==4) {
    strcpy(name,"Deep within Volcanic Mountain");
    strcpy(desc,"Your located in a dangerous volcanic rocky mountain range. There \
is no way to see a way out from here. Travel in any direction would either \
have you climbing a rocky cliff or falling down one. \
Shayol Ghul seems rather close.");
  }
  if (((exit[0]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[0]==SECT_ROAD)||(exit[0]==SECT_RUINS)){
    sprintf(buf,"You see %s to the north.",get_sector_name(exit[0]));
    strcat(desc,buf);strcpy(buf,"  ");
...
strcpy(desc,"You are in a dank swamps edge. The smell and humidity make this a repelling place to say the least. ");
  if (exitsum==4) { 
    ...
    strcpy(desc,"You are in a dank swampland. All around is murky water and decadent mud. Occasional trees and grass mar this sickeningly beautiful work of nature. ");
  }
  if (((exit[0]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[0]==SECT_ROAD)||(exit[0]==SECT_RUINS)){
    sprintf(buf,"You see %s to the north.",get_sector_name(exit[0]));
    strcat(desc,buf);strcpy(buf,"  ");
...
strcpy(desc,"You are at the edge of a jungle. An assortment of rare and unique plant life grow in abundance here, making it nearly impassable. ");
  if (exitsum==4) { 
    ...
    strcpy(desc,"You are deep within in a jungle, surrounded by extravagant tropical fauna. It seems there is no way out and no end to the wall of plant life in every direction. ");
  }
  if (((exit[0]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[0]==SECT_ROAD)||(exit[0]==SECT_RUINS)){
    sprintf(buf,"You see %s to the north.",get_sector_name(exit[0]));
    strcat(desc,buf);strcpy(buf,"  ");
