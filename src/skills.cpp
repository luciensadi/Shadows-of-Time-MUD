...
void do_gain (CHAR_DATA * ch, char *argument)
{
    ...
    if (!str_prefix (arg, "list"))
    {
        ...
        {
           send_to_char("You already have all the skills you need.\n\r", ch);
           return;
        } 
        ...
        for (gn = 0; gn < MAX_GROUP; gn++)
        {
            ...
            if (!ch->pcdata->group_known[gn]
                && group_table[gn].rating[ch->cClass] > 0)
            {
                sprintf (buf, "%-18s %-5d ",
                         group_table[gn].name,
                         ch->level >= LEVEL_HERO ?
                            group_table[gn].rating[ch->cClass] / 2 :
                            group_table[gn].rating[ch->cClass]);
                send_to_char (buf, ch);
                if (++col % 3 == 0)
                    send_to_char ("\n\r", ch);
            }
        ...
        for (sn = 0; sn < MAX_SKILL; sn++)
        {
                ...
                && skill_table[sn].spell_fun == spell_null)
            {
                sprintf (buf, "%-18s %-5d ",
                         skill_table[sn].name,
                         ch->level >= LEVEL_HERO ?
                            skill_table[sn].rating[ch->cClass] / 2 :
                            skill_table[sn].rating[ch->cClass]);
                send_to_char (buf, ch);
                if (++col % 3 == 0)
                    send_to_char ("\n\r", ch);
            }
    ...
    if (!str_prefix (arg, "study"))
    {
        ...
        act ("$N helps you apply your train to practices",
             ch, NULL, trainer, TO_CHAR);
        ch->practice += 10;
        ch->train -= 1;
        return;
    ...
    if (gn > 0)
    {
        ...
        if (group_table[gn].rating[ch->cClass] <= 0)
        {
        ...
        if ((ch->train < group_table[gn].rating[ch->cClass] && 
	      ch->level < LEVEL_HERO) ||
              (ch->train < group_table[gn].rating[ch->cClass] / 2))  
        {
    ...
    if (sn > -1)
    {
        ...
        if ((ch->train < skill_table[sn].rating[ch->cClass] &&
             ch->level < LEVEL_HERO) ||
             (ch->train < skill_table[sn].rating[ch->cClass] / 2))
        {
...
void do_spells (CHAR_DATA * ch, char *argument)
{
    ...
    for (sn = 0; sn < MAX_SKILL; sn++)
    {
        ...
        if ((level = skill_table[sn].skill_level[ch->cClass]) < LEVEL_HERO + 1
            && ((fAll && level <= MAX_LEVEL - 4) || level <= ch->level)
            && level >= min_lev && level <= max_lev
            && skill_table[sn].spell_fun != spell_null
            && ch->pcdata->learned[sn] > 0 &&
            (!grep || strstr(skill_table[sn].name, argument)) )
        {
            found = TRUE;
            level = skill_table[sn].skill_level[ch->cClass];
            if (ch->level < level)
...
int exp_per_level (CHAR_DATA * ch, int points)
{
    ...
    if (points < 60)
        return 2000 * (pc_race_table[ch->race].class_mult[ch->cClass] ?
                       pc_race_table[ch->race].class_mult[ch->cClass] /
                       100 : 1);
    ...
    expl += int((points * inc / 11) * 1.2);

    return expl * pc_race_table[ch->race].class_mult[ch->cClass] / 100;
}
...
bool parse_gen_groups (CHAR_DATA * ch, char *argument)
{
    ...
    if (!str_prefix (arg, "help"))
    {
        if (argument[0] == '\0')
        {
            do_function (ch, &do_mhelp, "* 'group help'");
            return TRUE;
        }
    ...
    if (!str_prefix (arg, "add"))
    {
        ...
        if (gn != -1)
        {
            ...
            if (group_table[gn].rating[ch->cClass] < 1)
            {
            ...
            if (ch->gen_data->points_chosen +
                group_table[gn].rating[ch->cClass] > 300)
            {
                send_to_char
            ...
            ch->gen_data->points_chosen += group_table[gn].rating[ch->cClass];
            gn_add (ch, gn);
            ch->pcdata->points += group_table[gn].rating[ch->cClass];
            return TRUE;
        }
        ...
        if (sn != -1)
        {
            ...
            if (skill_table[sn].rating[ch->cClass] < 1
                || skill_table[sn].spell_fun != spell_null
		|| skill_table[sn].rating[ch->cClass] < 0)
            {
            ...
            /* Close security hole */
            if (ch->gen_data->points_chosen +
                skill_table[sn].rating[ch->cClass] > 300)
            {
                send_to_char
                    ("You cannot take more than 300 creation points.\n\r",
                     ch);
            ...
            ch->gen_data->points_chosen += skill_table[sn].rating[ch->cClass];
            ch->pcdata->learned[sn] = 1;
            ch->pcdata->points += skill_table[sn].rating[ch->cClass];
            return TRUE;
        }
    ...
    {
        do_function (ch, &do_mhelp, "* premise");
        return TRUE;
    }
...
void do_groups (CHAR_DATA * ch, char *argument)
{
	...
	if
(skill_table[skill_lookup(group_table[gn].spells[sn])].rating[ch->cClass] <
1) continue;
...
void check_improve (CHAR_DATA * ch, int sn, bool success, int multiplier)
{
    ...
    chance /= (multiplier * abs(skill_table[sn].rating[ch->cClass]) * 4);
    chance += ch->level;
    ...
    if (success)
    {
        ...
        if (number_percent () < chance)
        {
            ...
            send_to_char (buf, ch);
            gain_exp (ch, 2 * abs(skill_table[sn].rating[ch->cClass]));
        }
    }
