...
bool has_said(CHAR_DATA *mob, CHAR_DATA *ch, long bit);
...
#define CHK_RANK        (52)
#define CHK_HASSAID     (53)
#define CHK_NOTSAID     (54)
#define CHK_HASQUEST    (55)
#define CHK_NOTHASQUEST    (56)
#define CHK_HAS_CMPLT_QUESTSTEP   (57)
#define CHK_NOTHAS_CMPLT_QUESTSTEP   (58)
#define CHK_HAS_CMPLT_QUEST (59)
#define CHK_NOTHAS_CMPLT_QUEST (60)
#define CHK_ONQUEST            (61)
#define CHK_NOTONQUEST            (62)
...
const char * fn_keyword[] =
{
    ...
    "rank",             /* if rank $n 9         - check to see if $n is of 9 rank*/
    "hassaid",          /* see if they've said */
    "notsaid",          /* see if they haven't said it yet */
    "hasquest",
    "!hasquest",
    "completedstep",
    "!completedstep",
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	case CHK_AFFECTED:
		...
		&&  STR_IS_SET(lval_char->affected_by, flag_lookup(buf, affect_flags)) );
	case CHK_ACT:
	    return( lval_char != NULL 
	...
	case CHK_RANK:
	    return( lval_char != NULL && lval_char->rank == atoi (buf));
        case CHK_HASSAID:
            return( has_said(mob, lval_char, atol(buf)) );
        case CHK_NOTSAID:
            return( !has_said(mob, lval_char, atol(buf)) );
        case CHK_HASQUEST:
            return( has_quest(lval_char, atoi(buf)));
        case CHK_NOTHASQUEST:
            return( !has_quest(lval_char, atoi(buf)));
        case CHK_ONQUEST:
            return( on_quest(lval_char, atoi(buf)));
        case CHK_NOTONQUEST:
            return( !on_quest(lval_char, atoi(buf)));
        case CHK_HAS_CMPLT_QUESTSTEP:
        {
          char step[MIL];
          line = one_argument(line, step);
          return (has_completed_quest_step(lval_char, atoi(buf), atoi(step)));
        }
        case CHK_NOTHAS_CMPLT_QUESTSTEP:
        {
          char step[MIL];
          line = one_argument(line, step);
          return (!has_completed_quest_step(lval_char, atoi(buf), atoi(step)));
        }
        case CHK_HAS_CMPLT_QUEST:
        {
          return (has_completed_quest(lval_char, atoi(buf)));
        }

        case CHK_NOTHAS_CMPLT_QUEST:
        {
          return (!has_completed_quest(lval_char, atoi(buf)));
        }
	default:;
...

{
    ...
    if ( mob && IS_AFFECTED(mob, AFF_SUPER_INVIS))
       return;
...
//	char *b = buf, *c = control, *d = data;
	...
	else if ( (!level || cond[level] == TRUE) && buf[0] != '\0' )
	{
		...
		if ( !room )
		    ...
		    iDelay = room_interpret( room, line );
	    }
	    else
	    {
		/* 
		 ...
		 */
		if ( !mob )
		    logf( "Normal MUD command in non-MOBprog, prog vnum %d %s", pvnum, data );
		else
		    interpret( mob, data );
	    }
            if (iDelay > 0)
            {
              call_level--;
              add_delay_prog(pvnum, source, mob, obj, room, ch, arg1, arg2, numLine++, iDelay);
              return;
            }
	}
    }
...
void add_delay_prog(
        ...
        OBJ_DATA *obj,
        ROOM_INDEX_DATA *room,
        CHAR_DATA *ch, const void *arg1, const void *arg2, int gotoLine, int iDelay )
{
  DELAY_CODE *prog;
  DELAY_CODE *list;
  ...
  if (delay_list->time > prog->time)
  {
    prog->next = delay_list;
    delay_list = prog;
    return;
  }

  for (list = delay_list;list != NULL; list = list->next)
  {
    if (!list->next)
    {
      prog->next = delay_list->next;
      list->next = prog;
      return;
    }

    if (prog->time < list->next->time)
    {
      prog->next = list->next;
      list->next = prog;
      return;
...
bool check_phrase(char *argument, char *phrase)
{ 
  char arg[MIL];
  if (phrase[0] != '*')
  {
    char temp[MIL];
    sprintf(temp, "%s", phrase);
    temp[0] = toupper(temp[0]);
    return (strstr(argument, phrase) || strstr(argument, temp));
  }

  char *clone;
  clone = str_dup(phrase);
  clone++;
  while (!IS_NULLSTR(clone))
  {
    logf("Making it.", 0);
    clone = one_argument(clone, arg);
    if (strstr(argument, arg) || strstr(argument, capitalize(arg)))
    {
      free_string(clone);
      return TRUE;
    }
	...
	const void *arg1, const void *arg2, int type )
{
	...
	for ( prg = mob->pIndexData->mprogs; prg != NULL; prg = prg->next )
	{
	    if ( prg->trig_type == type 
	      &&  check_phrase(argument, prg->trig_phrase))
	    {
		program_flow( prg->vnum, prg->code, mob, NULL, NULL, ch, arg1, arg2, 0 );
		break;
	    }
	}
    }
	...
	for ( prg = obj->pIndexData->oprogs; prg != NULL; prg = prg->next )
	{
	    ...
	    {
		program_flow( prg->vnum, prg->code, NULL, obj, NULL, ch, arg1, arg2, 0 );
		break;
	    }
	}
    }
	...
	for ( prg = room->rprogs; prg != NULL; prg = prg->next )
	{
	    if ( prg->trig_type == type
	      && strstr( argument, prg->trig_phrase ) != NULL )
	    {
		program_flow( prg->vnum, prg->code, NULL, NULL, room, ch, arg1, arg2, 0 );
		break;
	    }
	}
    }
...
void p_bribe_trigger( CHAR_DATA *mob, CHAR_DATA *ch, int amount )
{
	...
	{
	    program_flow( prg->vnum, prg->code, mob, NULL, NULL, ch, NULL, NULL, 0 );
	    break;
	}
    }
return;
...
bool p_exit_trigger( CHAR_DATA *ch, int dir, int type )
{
	...
	for ( obj = ch->in_room->contents; obj != NULL; obj = obj->next_content )
	{
		...
		for ( prg = obj->pIndexData->oprogs; prg; prg = prg->next )
		{
		    if ( prg->trig_type == TRIG_EXALL
		      && dir == atoi( prg->trig_phrase ) )
		    {
			program_flow( prg->vnum, prg->code, NULL, obj, NULL, ch, NULL, NULL, 0 );
			return TRUE;
		    }
		}
	    }
	...
	if ( HAS_TRIGGER_ROOM( room, TRIG_EXALL ) )
	{
		...
		if ( prg->trig_type == TRIG_EXALL
		    && dir == atoi( prg->trig_phrase ) )
		{
		    program_flow( prg->vnum, prg->code, NULL, NULL, room, ch, NULL, NULL, 0 );
		    return TRUE;
		}
	    }
	}
			...
			CHAR_DATA *ch, OBJ_DATA *dropped, int type )
{
	...
	for ( prg = mob->pIndexData->mprogs; prg; prg = prg->next )
		...
		if ( is_number( p ) )
		{
		    ...
		    {
			program_flow(prg->vnum, prg->code, mob, NULL, NULL, ch, (void *) dropped, NULL, 0);
			return;
		    }
		}
		/*
	...
	for ( prg = obj->pIndexData->oprogs; prg; prg = prg->next )
	    if ( prg->trig_type == type )
	    {
		program_flow( prg->vnum, prg->code, NULL, obj, NULL, ch, (void *) obj, NULL, 0 );
		return;
	    }
    }
    else if ( room )
	...
	for ( prg = room->rprogs; prg; prg = prg->next )
		...
		if ( is_number( p ) )
		{
		    if ( dropped->pIndexData->vnum == atoi(p) )
		    {
			program_flow( prg->vnum, prg->code, NULL, NULL, room, ch, (void *) dropped, NULL, 0 );
			return;
		    }
		}
		/*
