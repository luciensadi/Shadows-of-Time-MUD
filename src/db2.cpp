...
void load_mobiles (FILE * fp, int version)
{
    ...
    for (;;)
    {
        ...
        if (version >= 4)
        {
          pMobIndex->load_time.start = fread_number(fp);
          pMobIndex->load_time.end = fread_number(fp);
        }
        else
        {
          pMobIndex->load_time.start = 0;
          pMobIndex->load_time.end = 0;
        }
...
/*        pMobIndex->affected_by = fread_flag (fp)
        ...
        STR_OR_STR(pMobIndex->affected_by, race_table[pMobIndex->race].aff, 
                   AFF_FLAGS);
        ...
        /* read hit dice */
        pMobIndex->hit[DICE_NUMBER] = fread_long (fp);
        /* 'd'          */ fread_letter (fp);
        pMobIndex->hit[DICE_TYPE] = fread_long (fp);
        /* '+'          */ fread_letter (fp);
        pMobIndex->hit[DICE_BONUS] = fread_long (fp);
...
void load_objects (FILE * fp, int version)
{
    ...
    for (;;)
    {
	    ...
	    case ITEM_BOMB:
	        ...
	        pObjIndex->value[1] = explosive_lookup (fread_word (fp));
        ...
        for (;;)
        {
            ...
            else if ( letter == 'O' )
            {
                ...
                logf("Oprog Trig: %d", trigger);
                pOprog->vnum            = fread_long( fp );
            ...
            else if (letter == 'T')
            {
            	...
            	pObjIndex->timer_msg = fread_string(fp); 
            }        
            else
            {
                ungetc (letter, fp);
