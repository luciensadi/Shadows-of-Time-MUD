...
void check_couple args ((CHAR_DATA *ch, sh_int sn, void *vo, int target));
...
int spell_dam(int base,int number,int die)
{
    int dam;
    if (number < 1)
        number = 1;
    dam = base + dice(number,die);
    return dam;
}
...
void say_spell (CHAR_DATA * ch, int sn)
{
    ...
    {
      if (skill_table[sn].weaves[i] != 0)
        number += 1;
    }

    sprintf(buf, "You weave ");
    for (i = 0;i < MAX_WEAVES;i++)
    {
      if (skill_table[sn].weaves[i] != 0)
      {
        strcat(buf, weave_table[i]);
        count++;    
        if (count == number - 1 && number != 1)
          strcat(buf, " and ");
	...
	if (ch == rch)
	  continue;
        ...
        if (ch->sex == rch->sex || IS_IMMORTAL(rch))
        {
	  char send[MSL];
          sprintf(buf2, "into %s.", 
	     ch->cClass == rch->cClass ? skill_table[sn].name : 
	     IS_IMMORTAL(rch) ? skill_table[sn].name : "something unknown");
          sprintf(send, "%s%s", buf, buf2);
          if (!IS_SET(rch->act2, PLR_NOWEAVE))
            act (send, ch, NULL, rch, TO_VICT);
        }
        else
        {
          if (!IS_SET(rch->act2, PLR_NOWEAVE))
            act("You feel a tingling sensation.\n\r", rch, NULL, NULL, TO_CHAR);
        }
    }
 ...
 * Positive apply's make dispelling better
...
bool saves_spell (int level, CHAR_DATA * victim, int dam_type)
{
    ...
    mod_save = victim->saving_throw;
    mod_save += (level - victim->level);
       
    
    if (IS_NPC(victim))
       mod_save -= victim->level /2;
    ...
    if (!IS_NPC (victim) 
        && (class_table[victim->cClass].fMana==FMANA_CHANNEL ||class_table[victim->cClass].fMana==FMANA_BOTH))
        save = 9 * save / 7;

   save = URANGE (25, save, 105);
   if (!IS_NPC(victim) && IS_SET(victim->act2, PLR_TESTOR))
   {
   	sprintf(buf, "%d save throw\n\r", save);
   	act(buf,victim,NULL, victim,TO_ROOM);
   }
    return number_percent () > save;
}

int calc_spell_level(CHAR_DATA *ch, int spell)
{
  float value[MAX_WEAVES];
  float sum = 0;
  int iSum;
  int iReturn;
  int i;
  ...
  for (i = 0;i < MAX_WEAVES;i++)
  {
    value[i] = get_weave_stat(ch, i) * (skill_table[spell].weaves[i] / 10.0);
    sprintf(buf, "%d %2.2f ", i, value[i]);
//    if (IS_IMMORTAL(ch))
//      send_to_char(buf, ch); 
  }  
//  send_to_char("\n\r", ch);
...
//  if (IS_IMMORTAL(ch))
//    send_to_char(buf, ch);
  iReturn = ch->level + (iSum - 10);
  sprintf(buf, "iReturn %d\n\r", iReturn);
//  if (IS_IMMORTAL(ch))
//   send_to_char(buf, ch);
  return iReturn;
}
...
bool saves_dispel (int dis_level, int spell_level, int duration, int saves)
{
    int save;
    ...
    save = 10 + (spell_level - dis_level) * 4;
    save -= saves / 4;
...
bool check_dispel (int dis_level, CHAR_DATA * victim, int sn)
{
    ...
    if (is_affected (victim, sn))
    {
        for (af = victim->affected; af != NULL; af = af->next)
        {
            if (af->type == sn)
            {
                if (!saves_dispel (dis_level, af->level, af->duration, saves))
                {
...
void do_cast (CHAR_DATA * ch, char *argument)
{
    ...
    if (IS_AFFECTED(ch, AFF_STILL))
    {
      send_to_char("You have been stilled and cannot touch the source.\n\r", ch);
      return;
    }

    if ( (IS_SET(ch->in_room->room_flags, ROOM_NOMAGIC) || IS_SET(ch->in_room->area->area_flags, AREA_NO_MAGIC)) && (ch->pcdata->pk_timer == 0) && !IS_IMMORTAL(ch) )
    {
    	send_to_char( "This area appears to cut you off from the source.\n\r", ch);
    	return;
    }
    ...
    if (ch->level + 2 == skill_table[sn].skill_level[ch->cClass])
    {
    	if (ch->cClass==0)
    	    mana = 30;
    	else
...
*/
        ...
        case TAR_CHAR_DEFENSIVE:
            ...
            if (victim != ch && !IS_NPC(victim) && IS_SET (victim->act, PLR_TOURNEY))
            {
              send_to_char("They are on their own, it's a tournament.\n\r", ch);
              return;
            }

            if (!IS_NPC(victim))
              if (ch->clan == (clan_lookup("Tinker") && !IS_SET(ch->act2, PLR_TOURNEY)) && victim->pcdata->pk_timer >= 1 && victim != ch)
              {
                  send_to_char( "Tinkers should stay out of fights!\n\r", ch);
                  return;
              }   
            vo = (void *) victim;
            target = TARGET_CHAR;
        ...
        case TAR_OBJ_CHAR_DEF:
            ...
            else if ((victim = get_char_room (ch, NULL, target_name)) != NULL)
            {
                ...
                target = TARGET_CHAR;
                if (!IS_NPC(victim))                
                if (ch->clan == (clan_lookup("Tinker")) && victim->pcdata->pk_timer >= 1 && victim != ch)
                {
                  send_to_char( "Tinkers should stay out of fights!\n\r", ch);
                  return;
                }
    ...
    {
         if (!IS_NPC(ch) && IS_AFFECTED(ch, AFF_MANADRAIN2))
         {
            CHAR_DATA *temp;
            if ((temp = get_char_room (ch, NULL, ch->pcdata->manadrainee)) != NULL &&
                    IS_AFFECTED(temp, AFF_MANADRAIN))
            {
              if (temp->mana > mana / 2)
              {
                temp->mana -= mana / 2;
                mana /= 2;
                act("You draw energy from $N and use it in your weave.", ch, NULL, temp, TO_CHAR);
                act("You think you see a glow pass from $n to $N.", ch, NULL, temp, TO_NOTVICT);
                act("You feel $n sucking the source away from you.", ch, NULL, temp, TO_VICT);
              }
            }
         } 

        if (target == TARGET_CHAR && number_bits(3) < 2)
        {
          CHAR_DATA *victim = (CHAR_DATA *) vo;
          if (IS_AFFECTED(victim, AFF_MEDALLION)&& !furies_imm(victim,ch))
          {
            bool found = FALSE;
            CHAR_DATA *wch;
            for (wch = ch->in_room->people ; wch != NULL ; wch = wch->next_in_room)
            {
              if (number_bits(2) == 0 && wch != victim)
              {
                act("You feel a tingling sensation.", ch, NULL, victim, TO_VICT);
                act("Your spell is suddenly redirected.", ch, NULL, victim, TO_CHAR);        
                vo = (void *) wch;
                if (is_safe_spell(ch, wch, FALSE))
                  continue;                
                if (wch != ch)
                {
                  act("$n's spell is suddenly redirected and hits $N.", ch, NULL, wch, TO_NOTVICT);
                  act("$n's spell is suddenly redirected and hits you.", ch, NULL, wch, TO_VICT);
                }
                else
                {
                  act("$n's spell is suddenly redirected and hits $M.", ch, NULL, wch, TO_NOTVICT);
                  act("Your spell hits you!", ch, NULL, wch, TO_VICT);
                }
                found = TRUE; 
                break;
              }
              ...
              {
                act("You feel a tingling sensation.", ch, NULL, victim, TO_VICT);
                act("Your spell is suddenly dissipated.", ch, NULL, victim, TO_CHAR);
                return;
              }          
           }
        }
        ...
        ch->mana -= mana;
        if (IS_NPC (ch) 
            || ((class_table[ch->cClass].fMana==FMANA_CHANNEL) 
            ||class_table[ch->cClass].fMana==FMANA_BOTH))
            /* class has spells */
            level = calc_spell_level(ch, sn); 
        else
            level = 5 * calc_spell_level(ch, sn) / 6;
        if (!IS_NPC(ch) && ch->clan == clan_lookup("rebel") && ch->clan == clan_lookup("sedai"))
        {
    ...
    if (get_skill(ch, gsn_couple > 0))
    {
      check_couple(ch, sn, vo, target);
    }
 ...
 */
int obj_cast_spell (int sn, int level, CHAR_DATA * ch, CHAR_DATA * victim,
                     OBJ_DATA * obj)
{
    void *vo;
	...
	if ( IS_SET(ch->in_room->room_flags, ROOM_NOMAGIC) || IS_SET(ch->in_room->area->area_flags, AREA_NO_MAGIC))
    {
...
void spell_poison_missile (int sn, int level, CHAR_DATA * ch, void *vo,
                       int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
...
void spell_bless (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    {
        send_to_char("You cannot do that to this item!\n\r", ch);
        return;
    }    
        obj = (OBJ_DATA *) vo;
        ...
        if (IS_OBJ_STAT (obj, ITEM_EVIL))
        {
            ...
            if (!saves_dispel
                (level, paf != NULL ? paf->level : obj->level, 0, 0))
            {
                if (paf != NULL)
...
void spell_asmodeus_bless (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    CHAR_DATA *victim;
    OBJ_DATA *obj;
    AFFECT_DATA af;

    if (str_cmp(ch->name, "Demiscus"))
    {
    ...
    affect_to_char (victim, &af);
    send_to_char ("You feel the power of Demiscus.\n\r", victim);
    if (ch != victim)
        act ("You grant $N the favor of Demiscus.", ch, NULL, victim,
             TO_CHAR);
    return;
}
...
void spell_blindness (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    {
      send_to_char("You fail to blind them.\n\r", ch);
        return;
    }
    ...
    for (vch = char_list; vch != NULL; vch = vch_next)
    {
        ...
        if (vch->in_room == ch->in_room)
        {
            if (vch != ch && !is_safe_spell (ch, vch, TRUE))
                damage (ch, vch, dam, sn, DAM_LIGHTNING, TRUE, FALSE);
            continue;
        }
...
void spell_calm (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    {                            /* hard to stop large fights */
        for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room)
        {
            if (IS_NPC (vch) && (IS_SET (vch->imm_flags, IMM_MAGIC) ||
                                 IS_SET (vch->act, ACT_UNDEAD)) || is_safe_spell(ch, vch, TRUE))
            {
	        act("$N is unaffected by your calming words.", ch, NULL, vch, TO_CHAR); 
                return;
            }
            ...
            {
	        act("$N is way too irate affected by your calming words.", ch, NULL, vch, TO_CHAR); 
                return;
            }
	...
	if ( IS_NPC(vch) && vch->hunting != NULL )
	{
		vch->hunting = NULL;	
	}
//	if (IS_NPC(vch) && vch->memory != NULL )
//	{
//		mob_forget(vch,get_mem_data(vch,ch));
//	}
                         ...
                         int target)
{
    ...
    if (!IS_NPC(ch) && !IS_NPC(victim) && victim != ch && 
      (ch->pcdata->pk_timer > 0 || victim->pcdata->pk_timer > 0))
    {
      send_to_char("Not while in pk, use dispel.\n\r", ch);
      return;
    }
    ...
    {
        act ("$n looks less afraid.", victim, NULL, NULL, TO_ROOM);
        found = TRUE;
    }
    ...
    if (check_dispel (level, victim, skill_lookup ("frighten")))
    {
        act ("$n looks less frightened.", victim, NULL, NULL, TO_ROOM);
        found = TRUE;
    }
                        ...
                        int target)
{
    ...
    damage (ch, (CHAR_DATA *) vo, spell_dam(level*2,2,8), sn, DAM_HARM,
            TRUE, FALSE);
    return;
}
...
void spell_cause_critical (int sn, int level, CHAR_DATA * ch, void *vo,
                           int target)
{
    damage (ch, (CHAR_DATA *) vo, spell_dam(level*6,12,14), sn, DAM_HARM,
            TRUE, FALSE);
    return;
}
...
void spell_cause_serious (int sn, int level, CHAR_DATA * ch, void *vo,
                          int target)
{
    damage (ch, (CHAR_DATA *) vo, spell_dam(level*4,8,8), sn, DAM_HARM,
            TRUE, FALSE);
    return;
}
                            ...
                            int target)
{
    ...
    while (level > 0)
    {
             ...
             tmp_vict != NULL; tmp_vict = next_vict)
        {
            ...
            if (!is_safe_spell (ch, tmp_vict, TRUE) && (tmp_vict != last_vict) && !IS_TINKER(tmp_vict))
            {
                found = TRUE;
                last_vict = tmp_vict;
                       ...
                       int target)
{
    ...
    if (saves_spell (level, victim, DAM_OTHER))
    {
      send_to_char("They are too secure in their sexuality for that.\n\r", ch);
      return;
    }
    af.where = TO_AFFECTS;
    af.type = sn;
                         ...
                         int target)
{
    ...
    if (!IS_NPC(victim ))
    {
    	send_to_char ("Its not a good idea to charm players!\n\r", ch);
    	return;
    }

    if (count_charmed(ch) >= ch->level / 12)
    {
      send_to_char("You're already spending too much time charming others to attempt that.\n\r", ch);
      return;
    }
    ...
    if (IS_AFFECTED (victim, AFF_CHARM)
        ...
        || saves_spell (int(level*1.5), victim, DAM_CHARM))
      {
        send_to_char("You fail to be charming.\n\r", ch);
        return;
      }
    ...
    if (IS_NPC(victim) && victim->memory != NULL )
	{
		mob_forget(victim,get_mem_data(victim,ch));
	}
	
    if (!IS_SET(victim->act,ACT_NOHUNT))
        SET_BIT(victim->act,ACT_NOHUNT);
                           ...
                           int target)
{
    ...
    if (check_dispel (int(level * 1.2), victim, gsn_blindness))
    {
                          ...
                          int target)
{
    ...
    heal = 35+(dice (5, 5)) + level / 2;
    if (!IS_NPC(victim) && victim->pcdata->pk_timer > 0)
    {
      double tmp = heal * DAMAGE_REDUCER;
      heal = int(tmp); 
    }
                         ...
                         int target)
{
    ...
    if (check_dispel (int(level * 1.2), victim, gsn_plague))
    {
                        ...
                        int target)
{
    ...
    if (check_dispel (int(level * 1.2), victim, gsn_poison))
    {
...
void spell_curse (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    {
       send_to_char("Your words have no affect on them.\n\r", ch);
        return;
    }
    af.where = TO_AFFECTS;
    af.type = sn;
                         ...
                         int target)
{
    ...
    if (saves_spell (int(ch->level * 1.2), victim, DAM_OTHER))
    {
    ...
    if (IS_AFFECTED (victim, AFF_SANCTUARY)
        && !saves_dispel (level, victim->level, -1, 0)
    ...
    if (IS_AFFECTED (victim, AFF_MANASHIELD)
        && !saves_dispel (level, victim->level, -1, 0)
        && !is_affected (victim, skill_lookup ("mana shield")))
    {
        ...
        if (vch->in_room == ch->in_room)
        {
            if (vch != ch && !is_safe_spell (ch, vch, TRUE))
            {
                if (IS_AFFECTED (vch, AFF_FLYING))
                    ...
                    damage (ch, vch, spell_dam(level,8,12), sn, DAM_BASH, TRUE, FALSE);
            }
            continue;
        }
                          ...
                          int target)
{
    ...
    if (!obj->enchanted)
    {
        ...
        for (paf = obj->pIndexData->affected; paf != NULL ; paf = paf->next)
        {
            //Check if affect is a spell affect
                         ...
                         int target)
{
    ...
    if (victim != ch && 
           ((ch->level < LEVEL_HERO && !IS_DRAGON(ch) && !IS_FORSAKEN(ch))
            || !IS_SET(ch->act2, PLR_NOEXP)))
       ch->alignment = UMAX (-1000, ch->alignment - 50);
    ...
    for (tmp_vict = ch->in_room->people;tmp_vict != NULL; tmp_vict = next_vict)
        {
            ...
            if (!is_safe_spell (ch, tmp_vict, TRUE) && tmp_vict != last_vict)
            {
                found = TRUE;
                last_vict = tmp_vict;
                act ("A fireball bursts infront of you!", tmp_vict, NULL, NULL, TO_ROOM);
                act ("The fire hits you!", tmp_vict, NULL, NULL, TO_CHAR);
                level = UMIN (level,int(sizeof (dam_each) / sizeof (dam_each[0])) - 1);
                level = UMAX (0, level);
                dam = spell_dam(30,level/3,12);
                if (saves_spell (level, tmp_vict, DAM_FIRE))
                    dam /= 3;
                damage (ch, tmp_vict, dam, sn, DAM_FIRE, TRUE, FALSE);
                        ...
                        int target)
{
    ...
    {
       send_to_char("They already have that aura.\n\r", ch);
       return;
    }
    af.where = TO_AFFECTS;
    af.type = sn;
...
void spell_gate (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    if ((victim = get_char_world (ch, target_name)) == NULL
        ...
        || IS_SET (ch->in_room->area->area_flags, AREA_NO_SUMMON_FROM)
                           ...
                           int target)
{
    ...
    af.duration = level;
    af.location = APPLY_STR;
    af.modifier = 1 + (level >= 18) + (level >= 25) + (level >= 32) + (level >= 52) + (level >= 76);
    af.bitvector = 0;
    affect_to_char (victim, &af);
    af.location = APPLY_DAMROLL;
    affect_to_char (victim, &af);
...
void spell_harm (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    dam = spell_dam(level, level/2, 15);
	...
	if (!IS_NPC(victim) && victim->pcdata->condition[COND_BLEEDING] == 1 )
     {
	  ...
	  act ("$N's wounds close and bleeding stops!",ch,NULL,victim, TO_NOTVICT);
                          ...
                          int target)
{
    ...
    {
        int chlevel;
        chlevel = IS_HERO(ch) ? 110 : ch->level;
        if (!can_see_obj (ch, obj) || !is_name (target_name, obj->name)
            || IS_OBJ_STAT (obj, ITEM_NOLOCATE)
            || number_percent () > 2 * level || chlevel < obj->level
            || (obj->in_obj != NULL && IS_OBJ_STAT (obj->in_obj, ITEM_NOLOCATE)))
            continue;
        ...
        else
        {
            if (IS_IMMORTAL (ch) && in_obj->in_room != NULL)
                ...
                sprintf (buf, "%s is in %s's Store.\n\r", obj->short_descr,
                          in_obj->in_store->owner);
        }
                         ...
                         int target)
{
    ...
    for (gch = ch->in_room->people; gch != NULL; gch = gch->next_in_room)
    {
        if ((IS_NPC (ch) && IS_NPC (gch)) || (!IS_NPC (ch) && !IS_NPC (gch)))
        {
			...
			if (!IS_NPC(ch) && ch->pcdata->condition[COND_BLEEDING] == 1 )
			{
				SET_BLEED(ch,FALSE);
				act ("$N's wounds close and bleeding stops!",ch,NULL,NULL, TO_ROOM);
				send_to_char("Your wounds close and stops your bleeding.\n\r",ch);      
			} 
...
void spell_recharge (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    if (obj->value[4] == 0)
    {
        send_to_char ("That item has already been recharged.\n\r", ch);
        return;
    }
                         ...
                         int target)
{
    ...
    if (target == TARGET_OBJ)
    {
            ...
            //|| IS_OBJ_STAT (obj, ITEM_NOREMOVE))
        {
            if (!IS_OBJ_STAT (obj, ITEM_NOUNCURSE)
                && !saves_dispel (level + 2, obj->level, 0, 0))
            {
         ...
         obj = obj->next_content)
    {
        ...
        {                        /* attempt to remove curse */
            if (!saves_dispel (level, obj->level, 0, 0))
            {
                found = TRUE;
                REMOVE_BIT (obj->extra_flags, ITEM_NODROP);
                //REMOVE_BIT (obj->extra_flags, ITEM_NOREMOVE);
                         ...
                         int target)
{
    ...
    if (target == TARGET_OBJ)
    {
        ...
        if (IS_OBJ_STAT (obj, ITEM_NOREMOVE))
        {
            ...
            {
                REMOVE_BIT (obj->extra_flags, ITEM_NOREMOVE);
                act ("$p glows and fades.", ch, obj, NULL, TO_ALL);
                return;
            }

            act ("The enchantment on $p is beyond your power.", ch, obj, NULL,
                 TO_CHAR);
            return;
        }
        act ("There doesn't seem to be a enchantment on $p.", ch, obj, NULL,
             TO_CHAR);
        return;
    }
    ...
    for (obj = victim->carrying; (obj != NULL && !found);obj = obj->next_content)
    {
        if (IS_OBJ_STAT (obj, ITEM_NOREMOVE))
        { 
            if (!saves_dispel (level+2, obj->level, 0, 0)
                && !IS_OBJ_STAT (obj, ITEM_NOUNCURSE))
            {
                found = TRUE;
                REMOVE_BIT (obj->extra_flags, ITEM_NOREMOVE);
                act ("Your $p glows and fades.", victim, obj, NULL, TO_CHAR);
                act ("$n's $p glows and fades.", victim, obj, NULL, TO_ROOM);
            }
            else
            {
                act ("The enchantment on $p is beyond your power.", ch, obj, NULL,TO_CHAR);
                return;
            }
        }
    }
...
void spell_sanctuary (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    if (!str_cmp("warrior", class_table[victim->cClass].name))
    {
        if (victim == ch)
	        send_to_char ("You cannot use that spell.\n\r", ch);
	else
		act ("$N cannot use the power of sanctuary.", ch, NULL, victim, TO_CHAR);
        return;
    }
    ...
    if (IS_AFFECTED (victim, AFF_IRONSKIN))
    {
        ...
        {
        sprintf(buf,"%s is already maintaining iron skin to have sanctuary.\n\r", victim->name);
        send_to_char(buf,ch);
        }
        return;
    }
...
void spell_simulacrum (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    if (ch==victim)
    {
        send_to_char("You cannot target yourself with that spell.\n\r",ch);
        return;
    }
        
    heal = 40 + 5 * (dice (6,6)) + level;
    damage(ch, victim, heal, sn, DAM_NONE, TRUE, FALSE);
    heal = 40 + 3 * (dice (5, 6)) + level;
    if (!IS_NPC(ch) && ch->pcdata->pk_timer > 0)
    {
      double tmp = heal * DAMAGE_REDUCER;
      heal = int(tmp); 
    ...
    if ((ch->level < LEVEL_HERO && !IS_DRAGON(ch) && !IS_FORSAKEN(ch)
         || !IS_SET(ch->act2, PLR_NOEXP)) && !IS_SET(ch->act, PLR_QUESTING)) 
      ch->alignment = UMAX (-1000, ch->alignment - 25);
    update_pos (victim);
    act ("A warm feeling fills your body as you see $N stagger in pain!", ch, NULL, victim, TO_CHAR);
    WAIT_STATE(victim, PULSE_VIOLENCE);
  
	if (!IS_NPC(victim) && victim->pcdata->condition[COND_BLEEDING] == 1 )
     {
          SET_BLEED(victim,FALSE);
	  act ("$N's wounds close and bleeing stops!",ch,NULL,victim, TO_NOTVICT);
          send_to_char("Your wounds close and stops your bleeding.\n\r",victim);      
     } 
    return;
}
...
void spell_sleep (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    if (IS_AFFECTED (victim, AFF_SLEEP)
        || IS_AFFECTED (victim, AFF_BLACKJACK))
    {
    	send_to_char("Your victim is already asleep.",ch);
    	return;
    }

    if ((IS_NPC (victim) && IS_SET (victim->act, ACT_UNDEAD))
        || (level + 10) < victim->level
        || saves_spell (int(level * .9), victim, DAM_CHARM)) 
    {
    	send_to_char("Spell failed.",ch);
    	return;
    }
    ...
    if (IS_AWAKE (victim))
    {
        ...
        act ("$n suddenly falls asleep.", victim, NULL, NULL, TO_ROOM);
...
void spell_teleport (int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    ...
    if (IS_AFFECTED(ch,AFF_TELE_DISORIENT)&& !IS_IMMORTAL(victim))
    {
    	send_to_char ("You feel too disorientated to attempt that again.\n\r",ch);
    	return;
    }
    ...
    if (IS_MOUNTED(victim) && victim->mount)
    {
    ...
    af.duration = 2;
    af.modifier = 0;
    af.bitvector = AFF_TELE_DISORIENT;
    af.location = APPLY_NONE;
    affect_to_char (victim, &af);
    return;
}
                          ...
                          int target)
{
    ...
    sprintf (buf1, "%s says '`M%s`*'.\n\r", speaker, target_name);
    
    buf1[0] = UPPER (buf1[0]);

    for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room)
    {
    	sprintf (buf2, "%s attempted to mask their voice as %s says '`M%s'.\n\r", PERS(ch,vch,FALSE),speaker, target_name);
        if (!is_exact_name (speaker, vch->name) && IS_AWAKE (vch))
            send_to_char (number_percent()>85 ? buf2 : buf1,
                          vch);
    }
                           ...
                           int target)
{
    ...
    {
       send_to_char ("You cannot escape that easily.\n\r", ch);
       return;
    }
    ...
    if (IS_SET (victim->in_room->room_flags, ROOM_NO_RECALL) ||
        IS_AFFECTED (victim, AFF_CURSE) || IS_SET (victim->in_room->area->area_flags, AREA_NO_RECALL))
    {
    ...
    ch->move /= 2;
    act ("$n closes $s eyes and disappears.", victim, NULL, NULL, TO_ROOM);
    if (IS_MOUNTED(victim) && victim->mount)
    {
      char_from_room(victim->mount);
    ...
    act ("$n slowly appears in the room.", victim, NULL, NULL, TO_ROOM);
    do_function (victim, &do_look, "auto");
}
...
if ((IS_SET(ch->act, PLR_TOURNEY)) || (IS_SET(ch->act, PLR_TAG)) ||  IS_SET(ch->act, PLR_IT))
{
send_to_char ("Thats not fair during this game!\n\r", ch);
return;
}
    ...
    hpch = URANGE (12, ch->hit, 4000);
    hp_dam = number_range (hpch / 14 + 1, hpch / 12);
    dice_dam = number_range(level / 11, level / 9);

    dam = hp_dam + dice_dam + (ch->level / 5) - 20;

    if (saves_spell (int(level * 1.5), victim, DAM_ACID))
    {
                        ...
                        int target)
{
    ...
    hpch = URANGE (12, ch->hit, 4000);
    hp_dam = number_range (hpch / 13 + 1, hpch / 10);
    dice_dam = number_range(level / 11, level / 9);
    dam = hp_dam + dice_dam + (ch->level / 2 ) -15;
    fire_effect (victim->in_room, level, dam / 2, TARGET_ROOM);

            if (saves_spell (int(level * 1.5), victim, DAM_FIRE))
            {
                fire_effect (victim, level / 2, dam / 4, TARGET_CHAR);
                damage (ch, victim, dam / 2, sn, DAM_FIRE, TRUE, FALSE);
            }
            else
            {
                         ...
                         int target)
{
    ...
    cold_effect (victim->in_room, level, dam / 2, TARGET_ROOM);
        ...
        {                        /* full damage */
            if (saves_spell (int(level * 1.5), victim, DAM_COLD))
            {
                cold_effect (victim, level / 2, dam / 4, TARGET_CHAR);
                damage (ch, victim, dam / 2, sn, DAM_COLD, TRUE, FALSE);
            }
            else
            {
                       ...
                       int target)
{
  ...
  //  poison_effect (ch->in_room, level, dam, TARGET_ROOM);
    ...
    for (vch = ch->in_room->people; vch != NULL; vch = vch_next)
    {
        ...
        if (saves_spell (int(level * 1.5), vch, DAM_POISON))
        {
                             ...
                             int target)
{
    ...
    if (saves_spell (int(level * 1.5), victim, DAM_LIGHTNING))
    {
...
void spell_berry(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
    OBJ_DATA *berry;
    int power = int_app[number_fuzzy(get_curr_stat (ch, STAT_INT))].learn;

    power = int(power * 0.15);

    power += level*2;

    berry = create_object (get_obj_index (OBJ_VNUM_BERRY), level);
    berry->value[0] = int(level *.35);
    berry->value[1] = power;
    berry->timer = 2;
    berry->level = level - number_fuzzy(7); // I like my numbers fuzzy!
    obj_to_char (berry, ch);
    act("You put your hands together, and as you take them apart, $p appears.",
	ch,berry,0,TO_CHAR);
    act("$n puts $s hands together, and as $e takes them apart, $p appears.",
	ch,berry,0,TO_ROOM);
    return;
}

void spell_dimensional_alienation(int sn, int level, CHAR_DATA * ch, void
	*vo, int target)
{

    act("$n steps out of this dimension.", ch, 0, 0, TO_ROOM);
    stc("You step out of the normal dimension.\n\r", ch);
    ch->interdimension = number_range(get_curr_stat (ch, STAT_WIS)/8, get_curr_stat(ch, STAT_WIS)/4);
}
...
void spell_save_life( int sn, int level, CHAR_DATA *ch, void *vo, int target)	
{
    CHAR_DATA *victim;
    int life;
    ...
    life = ch->hit / 2;
    ch->hit /= 2;
    victim->hit = UMIN( victim->hit + int( life * .75 ), victim->max_hit );
    update_pos(victim);
...
void spell_rift(int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
 ...
 if( IS_AFFECTED(victim, AFF_RIFT))
 {
  send_to_char("They have already been rifted.\n\r", ch);
  return;
 }
 ...
 af.where     = TO_AFFECTS;
 af.type      = gsn_rift;
 af.level     = UMIN(101, level);
 af.duration  = 0;
 af.modifier  = -1;
 af.location  = APPLY_STR;
 af.bitvector = AFF_RIFT;
 affect_to_char(victim, &af);
 act("$N has been caught in the rift.", ch, NULL, victim, TO_CHAR);

 return;
}

void spell_disorient(int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
 CHAR_DATA *victim = (CHAR_DATA *)vo;
 AFFECT_DATA af;

 if( IS_AFFECTED(victim, AFF_DISORIENT))
 {
  send_to_char("They have already been disoriented.\n\r", ch);
  return;
 }
 ...
 affect_to_char(victim, &af);
 act("$N has been shot with a weave of air and spirit, disorienting them.", ch, NULL, victim, TO_CHAR);
 ...
 * Spell functions.
 */
void spell_fire_blossom (int sn, int level, CHAR_DATA * ch, void *vo,
                       int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
    ...
    {
	send_to_char("You can only use this at a third of health or less.\n\r", ch);
	ch->mana += number_range(100,200);
	return;
    }

    dam = spell_dam(135,level,50);
    damage (ch, victim, dam, sn, DAM_FIRE, TRUE, FALSE);
    return;
}
