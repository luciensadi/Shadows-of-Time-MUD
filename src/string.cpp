 ...
 ****************************************************************************/
char *string_replace (char *orig, char *old, char *nNew)
{
...
void string_add (CHAR_DATA * ch, char *argument)
{
    ...
    if (*argument == '.')
    {
	  ...
	  send_to_char("Spelling check completed.\n\r", ch);
          return;
        }  
	    ...
	    send_to_char (".i               - ispell the string   \n\r", ch);
	    send_to_char (".i <word>        - ispell the word     \n\r", ch);
...
char *fontnote (char *string, CHAR_DATA *ch, int type)
{
    ...
    {
        string = getline (string, tmpb);
        sprintf (buf2, "\e[1z<font '%s'>%s<font>\n\r", 
	         type == NOTE_RP ? ch->pcdata->fonts.rpnote : ch->pcdata->fonts.notes,
		 tmpb);
        strcat (buf, buf2);
    }
