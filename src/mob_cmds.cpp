...
const	struct	mob_cmd_type	mob_cmd_table	[] =
{
    ...
    {	"flee",		do_mpflee	},
    {	"remove",	do_mpremove	},
    {   "addsaid",      do_mpaddsaid    },
    {   "addquest",     do_mpaddquest   },
    {   "completequeststep", do_mpcompletequeststep },
    {   "updatequeststatus", do_mpupdatequeststatus },
    {   "randomoload", do_mprandomoload },
    {	"",		0		}
};
 ...
 */
int mob_interpret( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_cmp(command, "delay"))
    {
      if (!is_number(argument))
      {
        bug("Delay with no number", 0);
        return 0;
      }
      else
...
void do_mpgecho( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
	bug( "MpGEcho: missing argument from vnum %ld",
...
void add_said(CHAR_DATA *mob, CHAR_DATA *ch, long bit)
{
  SAID_TO_DATA *list = NULL;
  bool found = FALSE;

  if (mob->saidto != NULL)
  {
    for (list = mob->saidto;list != NULL;list = list->next)
    {
      if (list->ch_id == ch->id)
      {
        found = TRUE;
  ...
  if (found)
  {
     if (!SET_BIT(list->bit, long(pow(2, bit)) ))
       SET_BIT(list->bit, long(pow(2, bit)));
  }
  else
  {
     list = new SAID_TO_DATA;
     list->ch_id = ch->id;
     list->bit = 0;
     SET_BIT(list->bit, long(pow(2, bit)));
     list->next = mob->saidto;
     mob->saidto = list;
  }
}
...
bool has_said(CHAR_DATA *mob, CHAR_DATA *ch, long bit)
{
  ...
  if (!found)
    ...
    return IS_SET(list->bit, long(pow(2, bit)));
}
...
/*
 * Allow the mob to load a random object.
 * Added by Diem
 * Syntax: mob randomoload [vnum] [vnum] [level] {R}
*/
void do_mprandomoload( CHAR_DATA *ch, char *argument )
{
     ...
     */
    if ( arg4[0] == 'R' || arg4[0] == 'r' )
	fToroom = TRUE;
    else if ( arg4[0] == 'W' || arg4[0] == 'w' )
	fWear = TRUE;

    int rand;
    rand = number_range(atoi(arg1), atoi(arg2));
    if ( ( pObjIndex = get_obj_index(  rand  ) ) == NULL )
    {
...
void do_mpcall( CHAR_DATA *ch, char *argument )
{
    ...
    program_flow( prg->vnum, prg->code, ch, NULL, NULL, vch, (void *)obj1, (void *)obj2, 0 );
}
...
void do_mpaddquest( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char arg[ MAX_INPUT_LENGTH ];
    ...
    add_quest(victim, atoi(argument));
}

void do_mpupdatequeststatus( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char arg[ MAX_INPUT_LENGTH ];
    char quest[ MAX_INPUT_LENGTH ];

    argument = one_argument( argument, arg );
    argument = one_argument( argument, quest );
    if ( ( victim = get_char_room( ch, NULL, arg ) ) == NULL )
	return;
    ...
    update_quest_status(victim, atoi(quest), argument);
}

void do_mpcompletequeststep( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char arg[ MAX_INPUT_LENGTH ];
    char questnum[ MAX_INPUT_LENGTH ];
    ...
    argument = one_argument(argument, questnum);
    if (!is_number(questnum) || !is_number(argument))
    {
      logf("Complete_step: Invalid argument.", 0);
      return;
    }

    quest_completed_step(victim, atoi(questnum), atoi(argument));
}


void do_mpaddsaid( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    long bit;
    char arg[ MAX_INPUT_LENGTH ];
 ...
 * reasons. A trivial hack of interpret()
 */
int obj_interpret( OBJ_DATA *obj, char *argument )
{
    char buf[MAX_STRING_LENGTH], command[MAX_INPUT_LENGTH];
    int cmd;
...
void do_opechoat( OBJ_DATA *obj, char *argument )
{
    ...
    act_new( argument, victim, obj, victim, TO_CHAR, POS_DEAD );
}
...
void do_opmload( OBJ_DATA *obj, char *argument )
{
    ...
    if ( ( pMobIndex = get_mob_index( vnum ) ) == NULL )
    {
	sprintf( arg, "Opmload: bad mob index (l%d) from obj %ld",
...
void do_opcall( OBJ_DATA *obj, char *argument )
{
    ...
    program_flow( prg->vnum, prg->code, NULL, obj, NULL, vch, (void *)obj1, (void *)obj2, 0 );
}
 ...
 * reasons. A trivial hack of interpret()
 */
int room_interpret( ROOM_INDEX_DATA *room, char *argument )
{
    char buf[MAX_STRING_LENGTH], command[MAX_INPUT_LENGTH];
    int cmd;
...
void do_rpmload( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    if ( ( pMobIndex = get_mob_index( vnum ) ) == NULL )
    {
	sprintf( arg, "Rpmload: bad mob index (%ld) from room %ld",
	    vnum, room->vnum );
	bug( arg, 0 );
	return;
    }
...
void do_rpcall( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    program_flow( prg->vnum, prg->code, NULL, NULL, room, vch, (void *)obj1, (void *)obj2, 0 );
}
