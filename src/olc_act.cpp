...
/* ROM specific bits: */
    ...
    {"trigger", trigger_flags, "Trigger flags."},
    {"ingredient", ingredient_flags, "Ingredient flags."},
    {"state", state_flags, "Bomb state flags."},
    {"explosion", explosion_flags, "Explosion flags."},
...
REDIT (aedit_area)
{
    ...
    {
        send_to_char ("Sintaxis: area [flags]\n\r", ch);
        return FALSE;
    }
...
AEDIT (aedit_vnum)
{
    ...
    if (iupper > 65000)
    {
      send_to_char("Upper range set at 65k rooms.\n\r", ch);
      return FALSE;
    }
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
        ...
        case ITEM_PEN:
            sprintf (buf,
                     "[v0] Number of Uses      [%ld]\n\r"
                     "[v1] Max Number of Uses  [%ld]\n\r",
                     obj->value[0],
                     obj->value[1]);
            send_to_char(buf,ch);
            break;
                     ...
                     char *argument)
{
    switch (pObj->item_type)
    {
        ...
        case ITEM_STAFF:
            switch (value_num)
            {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    send_to_char ("RECHARGES CHARGES SET.\n\r\n\r",
                                  ch);
        ...
        case ITEM_PILL:
            ...
            {
                default:
                    do_mhelp (ch, "* ITEM_SCROLL_POTION_PILL");
                    return FALSE;
                case 0:
                    send_to_char ("SPELL LEVEL SET.\n\r\n\r", ch);
                    pObj->value[0] = atoi (argument);
...
/* WEAPONS changed in ROM */
        ...
        case ITEM_TOKEN:
            ...
            {
                default:
                    do_mhelp( ch, "* ITEM_TOKEN" );
                    return FALSE;
                
                case 0:
                    send_to_char( "Qps Value Set.\n\r\n\r", ch );
                    pObj->value[0] = atoi( argument );
                    break;
            }
            break;
        ...
        case ITEM_BOMB:
            ...
            {
                default:
                    do_mhelp (ch, "* ITEM_BOMB");
                    return FALSE;

                case 0:
                    send_to_char ("Trigger Flag Set.\n\r\n\r", ch);
                    ALT_FLAGVALUE_TOGGLE (pObj->value[0], trigger_flags,
                                          argument);                          
                    break;
                case 1:
                    send_to_char ("Explosive Type Set.\n\r\n\r", ch);
                    pObj->value[1] = explosive_lookup(argument);  
                    break;
                case 2:
                    send_to_char ("State Flag Set.\n\r\n\r", ch);
                    ALT_FLAGVALUE_TOGGLE (pObj->value[2], state_flags,
                                          argument);                          
                    break;
                case 3:
                    send_to_char ("Ave dam Set.\n\r\n\r", ch);
                    pObj->value[3] = atoi(argument);
                    break;
         ...
         case ITEM_INGREDIENT:
            ...
            {
                default:
                    do_mhelp (ch, "* ITEM_INGREDIENT");
                    return FALSE;

                case 0:
                    send_to_char ("Ingredient Flag Set.\n\r\n\r", ch);
                    ALT_FLAGVALUE_TOGGLE (pObj->value[0], ingredient_flags,
                                          argument); 
                    break;
            }
            break;
        ...
        case ITEM_FIRE:
            ...
            {
                default:
                    do_mhelp (ch, "* ITEM_FIRE");
                    return FALSE;

                case 0:
                    send_to_char ("Duration.\n\r\n\r", ch);
                    pObj->value[0] = atoi (argument);
                    break;
            }
            break;
        ...
        case ITEM_QUIVER:
            switch (value_num)
            {
            	...
            	case 0:
            	    ...
            	    break;
                case 1:
                    send_to_char("MAX # OF ARROWS SET.\n\r",ch);
                    pObj->value[1] = atoi(argument);
                    break;
                case 2:
                    send_to_char("RESERVED SET.\n\r",ch);
            	    pObj->value[2] = atoi(argument);
                    break;
                case 3:
	   ...
	   case ITEM_KEYRING:
            switch (value_num)
            {
                ...
                default:
                    return FALSE;
                case 0:
                    send_to_char ("Key Capacity CAPACITY SET.\n\r\n\r", ch);
                    pObj->value[0] = atoi (argument);
                    break;
            }
            break;
        ...
        case ITEM_DRINK_CON:
            ...
            {
                default:
                    do_mhelp (ch, "* ITEM_DRINK");
/* OLC            do_mhelp( ch, "* liquids" );    */
                    return FALSE;
                case 0:
                    send_to_char
...
/* OLC            do_mhelp( ch, "* liquids" );    */
       ...
       case ITEM_TRAP:
        ...
        case ITEM_PIPE_TABAC:
            ...
            {
                default:
                    send_to_char( "set v0 to the amount of tabac\n\r", ch);
                    return FALSE;
                case 0:
                    send_to_char( "Amount of tabac SET.\n\r", ch );
                    pObj->value[1] = atoi( argument );
                    break;
            }
	    ...
	    switch ( value_num )
	    {
			int value;
	        default:
		    send_to_char( "set v0 to the destination room, or to zero if you wish to use the windows description. \n\rSet v1 to the standard container open/closed flags(only closed, closeable, and locked work(DUH!)).\n\rSet v2 to 1 if you don't want anybody to be able to crawl through the window.\n\r",ch);
	            return FALSE;
	        case 0:
                ...
                send_to_char ("Window Status Set.\n\r\n\r", ch);
                break;
			case 2:
				send_to_char("Passable status set.\n\r\n\r", ch);
				pObj->value[2] = atoi( argument );
				break;
		}
	break;
...
OEDIT (oedit_show)
{
    ...
    sprintf(buf, "\n\r`$Allowed Pnts: `&%d\n\r"
		 "`$Total Pnts:   `&%d\n\r"
		 "`$Affect Pnts:  `&%d  `$Extra Pnts:  `&%d  `$Weapon Pnts:  `&%d`*\n\r\n\r",
            calculate_allowed(pObj),
            calculate_points(pObj),
	    calculate_affect_points(pObj),
	    calculate_flag_points(extra_flags, pObj->extra_flags),
	    pObj->item_type == ITEM_WEAPON ? calculate_flag_points(weapon_type2, pObj->value[4]) : 0 );
    send_to_char(buf, ch);
    show_obj_values (ch, pObj);
...
OEDIT (oedit_addspell)
{
    ...
    if (!is_number(argument))
    {
      send_to_char("Syntax is addspell <spell> <duration>\n", ch);
      return FALSE;
    }
    ...
    for ( pos = 0; affect_flags[pos].name != NULL; pos++ )
      if (!str_prefix(arg, affect_flags[pos].name))
      {
        bv = affect_flags[pos].bit;
        found = TRUE;    
      }
    ...
    pAf->type = skill_lookup(arg);
    pAf->location = APPLY_SPELL_AFFECT;
    pAf->modifier = 0;
    pAf->duration = duration;
    pAf->bitvector = bv;
    pAf->level = URANGE(20,pObj->level,80);
    pAf->next = pObj->affected;
    pObj->affected = pAf;
...
OEDIT (oedit_honorlevel)
{
   ...
   if (argument[0] == '\0' || !is_number (argument))
   {
        send_to_char ("Syntax: honorlevel [number]\n\r", ch);
        return FALSE;
   }
    
   pObj->honor_level = atoi (argument);
   
   send_to_char("Honorlevel set.\n\r", ch);
   return TRUE;
}
...
OEDIT (oedit_timermsg)
{
    ...
    if (argument[0] == '\0')
    {
        send_to_char ("Syntax:  timermsg [string]\n\r", ch);
        return FALSE;
    }

    free_string (pObj->timer_msg);
    pObj->timer_msg = str_dup (argument);
    pObj->timer_msg[0] = UPPER (pObj->timer_msg[0]);

    send_to_char ("Timer Message set.\n\r", ch);
    return TRUE;
}
/*
...
/* ROM values: */
    ...
    sprintf (buf, "Load time: %d-%d\n\r", pMob->load_time.start, pMob->load_time.end);
    send_to_char (buf, ch);
...
MEDIT (medit_load_time)
{
    ...
    if (argument[0] == '\0' || arg[0] == '\0' ||    
        !is_number (argument) || !is_number (arg))
    {
        send_to_char ("Syntax:  load [start] [end] in 24 hour clock hours only\n\r", ch);
        return FALSE;
    }
    ...
    {
      send_to_char("All values must be between 0 and 23\n\r", ch);
      return FALSE;
    }

    pMob->load_time.start = atoi(arg);
    pMob->load_time.end = atoi(argument);


    send_to_char ("Load time set.\n\r", ch);
    return TRUE;
}
...
MEDIT( medit_copy )
{
    ...
    STR_COPY_STR( pMob->affected_by, pMob2->affected_by, AFF_FLAGS);   
...
GEDIT (gedit_show)
{
  ...
  EDIT_GUILD(ch, guild);
  sprintf(buf, "Guild name: %s\n\r", guild->name);
  send_to_char(buf, ch);
  sprintf(buf, "Guild who name: %s\n\r", guild->who_name);
  send_to_char(buf, ch);
  sprintf(buf, "Guild display name: %s\n\r", guild->display);
  send_to_char(buf, ch);
  send_to_char("Ranks:\n\r", ch);
  for (i = 0; i < guild->top_rank; i++)
  {
    sprintf(buf, "%2d: %s\n\r", i, guild->rank[i]);
    send_to_char(buf, ch);
  }
  send_to_char("Gskills:\n\r", ch);
  for (i = 0; i < guild->top_gskill; i++)
  {
    sprintf(buf, "%2d: %s\n\r", i, guild->gskill[i]);
    send_to_char(buf, ch);
  }
...
GEDIT (gedit_name)
{
  ...
  if (argument[0] == '\0')
  {
    send_to_char("You must enter a name in all lowercase\n\r", ch);
    return FALSE;
  }
  
  argument[0] = LOWER(argument[0]);
  free_string(guild->name);
  guild->name = str_dup(argument);
...
GEDIT (gedit_who)
{
  ...
  free_string(guild->who_name);
  guild->who_name = str_dup(argument);
  send_to_char("Who name changed.\n\r", ch);
  return TRUE;
}
...
GEDIT (gedit_display)
{
  ...
  free_string(guild->display);
  guild->display = str_dup(argument);
  send_to_char("Display name changed.\n\r", ch);
  return TRUE;
}
...
GEDIT (gedit_create)
{
  CLAN_DATA *guild;
  EDIT_GUILD(ch, guild);
  if ((top_clan + 1) == MAX_CLAN)
  {
    send_to_char("No more clans available.\n\r", ch);
    return FALSE;
  }
  clan_table[top_clan].name = str_dup("newclan");
  clan_table[top_clan].who_name = str_dup("[NEWCLAN]");
  clan_table[top_clan].vnum = 3001;
  clan_table[top_clan].display = str_dup("NewClan");
  clan_table[top_clan].top_rank = 0;
  clan_table[top_clan].top_gskill = 0;
  clan_table[top_clan].roster.member = NULL;
  clan_table[top_clan].roster.questpoints = 0;
  clan_table[top_clan].roster.withdraw = -1;
  clan_table[top_clan].roster.approve = FALSE;
  clan_table[top_clan].roster.requestor = FALSE;
  guild = &clan_table[top_clan];
  ch->desc->pEdit = (void *) guild;
  top_clan++;

  send_to_char("New guild created.\n\r", ch);
  return TRUE;
}
...
GEDIT (gedit_rank)
{
  ...
  if (!str_cmp(arg1, "add"))
  {
    if (guild->top_rank == MAX_RANK)
    {
      send_to_char("No more ranks available.\n\r", ch);
      return FALSE;
    }
    guild->top_rank++;
    guild->rank[guild->top_rank - 1] = str_dup("New Rank");
    send_to_char("Rank added now edit it.\n\r", ch);
    return TRUE;
  }
  else if (!str_cmp(arg1, "delete"))
  {
    free_string(guild->rank[guild->top_rank - 1]);
    guild->top_rank--;
  ...
  {
    send_to_char("Choose a rank #\n\r", ch);
    return FALSE;
  }
  num = atoi(arg1);
  if (num < 0 || num >= guild->top_rank)
  {
    char buf[MIL];
    sprintf(buf, "Choose a rank between 0 and %d\n\r", guild->top_rank - 1);
    send_to_char(buf, ch);
    return FALSE;
  }
  free_string(guild->rank[num]);
  guild->rank[num] = str_dup(argument);
  return TRUE;
}

GEDIT (gedit_commands)
{
  CLAN_DATA *guild;
  EDIT_GUILD(ch, guild);
...
GEDIT (gedit_delete)
{
  ...
  if (!is_number(arg1))
  {
    ...
    for (i = 0; i < top_clan; i++)
    {
      if (!str_cmp(arg1, clan_table[i].name))
      {
        found = TRUE;
        break;
      }
  ...
  else
  {
   ...
   {
     char buf[MSL];
     sprintf(buf, "Are you sure you want to delete %s\n\r", 
		 clan_table[clan].display);
     send_to_char(buf, ch);
     send_to_char("If so, repeat previous syntax post-fixed with yes.\n\r", ch);
     return FALSE;
   }


   // Get rid of current guildees
   for (d = descriptor_list; d != NULL; d = d->next)
   {
     ...
     if (wch->clan == clan)
       wch->clan = clan_lookup("Outcast");
   }

   top_clan--;
   for (i = clan; i < top_clan; i++)
   {
     free_string(clan_table[i].name);
     free_string(clan_table[i].who_name);
     free_string(clan_table[i].display);
     for (j = 0; j < clan_table[i].top_rank; j++)
     {
       free_string(clan_table[i].rank[j]);
     }

     for (j = 0; j < clan_table[i].top_gskill; j++)
     {
       free_string(clan_table[i].gskill[j]);
     }
     clan_table[i].name = str_dup(clan_table[i+1].name);
     clan_table[i].who_name = str_dup(clan_table[i+1].who_name);
     clan_table[i].display = str_dup(clan_table[i+1].display);
     clan_table[i].top_rank = clan_table[i+1].top_rank;
     for (j = 0; j < clan_table[i].top_rank; j++)
     {
       clan_table[i].rank[j] = str_dup(clan_table[i+1].rank[j]);
     }

     clan_table[i].top_gskill = clan_table[i+1].top_gskill;
     for (j = 0; j < clan_table[i].top_gskill; j++)
     {
       clan_table[i].gskill[j] = str_dup(clan_table[i+1].gskill[j]);
     }
     clan_table[i].roster = clan_table[i+1].roster;
  }
  return TRUE;
}
...
QEDIT ( qedit_create )
{
  ...
  if ((top_qprize + 1) == MAX_PRIZE)
  {
    send_to_char("No more QPrizes available.\n\r", ch);
    return FALSE;
  }
 
  qprize_table[top_qprize].keyword = str_dup("NewItem");
  qprize_table[top_qprize].showname = str_dup("NewItem");
  qprize_table[top_qprize].level = 0;
  qprize_table[top_qprize].vnum = 0;
  qprize_table[top_qprize].cost = 0;
  qprize_table[top_qprize].special = TRUE;

  quest = &qprize_table[top_qprize];
  ch->desc->pEdit = (void *) quest;
  top_qprize++;
  send_to_char("New Quest Prize Item added.\n\r", ch);
  return TRUE;
}
...
QEDIT ( qedit_keyword )
{
  QPRIZE_DATA *quest;
  EDIT_QUEST(ch, quest);
  free_string(quest->keyword);  
  quest->keyword = str_dup(argument);
  send_to_char("Keyword changed\n\r", ch);
  return TRUE;
}

QEDIT ( qedit_showname )
{
  QPRIZE_DATA *quest;
  EDIT_QUEST(ch, quest);

  free_string(quest->showname);  
  quest->showname = str_dup(argument);
  send_to_char("Showname changed\n\r", ch);
...
QEDIT ( qedit_cost )
{
  ...
  quest->cost = atoi(argument);
  send_to_char("Cost changed.\n\r", ch);
  return TRUE;
}
...
QEDIT ( qedit_vnum )
{
  ...
  quest->vnum = atoi(argument);
  send_to_char("Vnum changed.\n\r", ch);
  return TRUE;
}
...
QEDIT ( qedit_special )
{
  ...
  {
    quest->special = TRUE;
    send_to_char("Special set to TRUE\n\r", ch);
  }
  else if (!str_cmp(argument, "no"))
  {
    quest->special = FALSE;
    send_to_char("Special set to FALSE\n\r", ch);
  }
  else
  {
    send_to_char("Enter yes or no\n\r", ch);
  }
  return TRUE;
}
...
QEDIT ( qedit_show )
{
  ...
  sprintf(buf, "QPrize Keyword: %s\n\r", quest->keyword);
  send_to_char(buf, ch);
  sprintf(buf, "QPrize ShowName: %s\n\r", quest->showname);
  send_to_char(buf, ch);
  sprintf(buf, "QPrize Level: %d\n\r", quest->level);
  send_to_char(buf, ch);
  sprintf(buf, "QPrize Vnum: %ld\n\r", quest->vnum);
  send_to_char(buf, ch);
  sprintf(buf, "QPrize Cost: %d\n\r", quest->cost);
  send_to_char(buf, ch);
  sprintf(buf, "QPrize Special: %s\n\r", quest->special ? "True" : "False");
  send_to_char(buf, ch);
  return TRUE;
}
