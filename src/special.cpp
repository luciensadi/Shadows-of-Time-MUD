...
DECLARE_SPEC_FUN (spec_breath_any_but_acid);
...
DECLARE_SPEC_FUN (spec_illuminator);
DECLARE_SPEC_FUN (spec_rescue);
DECLARE_SPEC_FUN (spec_alice);


/* the function table */
const struct spec_type spec_table[] = {
    {"spec_breath_any", spec_breath_any},
    {"spec_breath_any_but_acid", spec_breath_any_but_acid},
    ...
    {"spec_illuminator", spec_illuminator},
    {"spec_rescue", spec_rescue},  
    {"spec_alice", spec_alice},  
    {NULL, NULL}
};
...
bool spec_alice( CHAR_DATA *ch )
{
  return FALSE;
}

bool spec_rescue( CHAR_DATA *ch )
{
  char buf[MSL];
  ...
  if (number_bits(3) == 0 && ch->rescuer && !IS_NULLSTR(ch->rescuer->name))
  {
    sprintf(buf, "Help me %s, you're my only hope.", ch->rescuer->name);
...
bool spec_assassin( CHAR_DATA *ch )
{
   ...
   if ( rnd_say <= 5)
                ...
                sprintf( buf, "Death comes swiftly....");
   else if ( rnd_say <= 10)
...
bool spec_breath_any_but_acid (CHAR_DATA * ch)
{
    if (ch->position != POS_FIGHTING)
        return FALSE;
...
bool spec_illuminator (CHAR_DATA * ch)
{
    CHAR_DATA *victim;
    CHAR_DATA *v_next;
    OBJ_DATA *bomb;
    char buf[MSL];
    int bombNumber;
//    logf("Illuminator called by %s", ch->short_descr);
    ...
    for (;;)
    {
        ...
        switch (number_range(0, 4))
        {
            ...
            case 3:
                min_level = 55;
                bombNumber = 9;
                // Shaped Contact Explosive
                break;
            case 4:
                min_level = 60;
                bombNumber = 10;
                // Poison Contact Explosive
                break;
        }

//        logf("Illum: Bomb %d", bombNumber);
        if (ch->level >= min_level)
            break;
    }
     ...
     bomb->value[3] = bomb_table[bombNumber].avedam + (ch->level * 3) + number_range(1, 50);
     ...
     obj_to_char(bomb, ch);
     act("$n has made $p.", ch, bomb, NULL, TO_ROOM);

     if ((number_percent() > bomb_table[bombNumber].stability) && 
         (number_percent() > 3 * UMIN(ch->level * 2, 100) / 4))
     {
	send_to_char("Oops!\n\r", ch);
     ...
     sprintf(buf, "'%s' %s", bomb_table[bombNumber].name, victim->name);
//     logf("Illum: Throw %s", buf);
     do_function(ch, &do_throw, buf);
     return TRUE;
}
...
bool spec_poison (CHAR_DATA * ch)
{
    ...
    if (ch->position != POS_FIGHTING
        ...
        || number_percent () > ch->level * 5) return FALSE;
