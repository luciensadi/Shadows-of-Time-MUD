...
char *olc_ed_vnum (CHAR_DATA * ch)
{
    ...
    switch (ch->desc->editor)
    {
        ...
        case ED_HELP:
            ...
            break;
        case ED_GUILD:
            pGuild = (CLAN_DATA *) ch->desc->pEdit;
            sprintf (buf, "%s", pGuild ? pGuild->name : "");
            break;
        default:
            sprintf (buf, " ");
            break;
...
/*  {   command        function    }, */
    ...
    {"name", gedit_name},
    {"who", gedit_who},    /* s removed -- Hugin */
    {"display", gedit_display},
    {"commands", show_commands},
    {"list", gedit_list},
    {"rank", gedit_rank},
    {"delete", gedit_delete},
...
/*  {   command        function    }, */
    ...
    {"show", qedit_show},
    {"showname", qedit_showname},
    {"cost", qedit_cost},
    {"vnum", qedit_vnum},
    {"level", qedit_level},
    {"create", qedit_create},
    {"list", qedit_list},
    {"special", qedit_special},
    {"delete", qedit_delete},
    {"commands", show_commands},
    {NULL, 0},
};
...
void do_aedit (CHAR_DATA * ch, char *argument)
{
    ...
    {
      send_to_char("You don't have high enough security.\n\r", ch);
      return;
    }       
...
void do_gedit (CHAR_DATA * ch, char *argument)
{
    ...
    {           
	  ...
	  int num;
          ...
          if (num < 0 || num >= top_clan)
          {
            send_to_char("That guild doesn't exist.\n\r", ch);
            return;
          } 

          guild = &clan_table[num];
        }
	else
	{
	  int i;
	  ...
	  if (!found)
	  {
	    send_to_char("No such guild found.\n\r", ch);
	    return;
	  }
	}
    }
...
void do_qedit (CHAR_DATA * ch, char *argument)
{
    ...
    {           
	  ...
	  int num;
          ...
          if (num < 0 || num >= top_qprize)
          {
            send_to_char("That qprize doesn't exist.\n\r", ch);
            return;
          } 

          quest = &qprize_table[num];
        }
	else
	{
	  int i;
	  ...
	  {
	    if (!str_cmp(arg1, qprize_table[i].keyword))
	    {
	      quest = &qprize_table[i];
	      found = TRUE;
	      break;
	    }
          }
	  if (!found)
	  {
	    send_to_char("No such qprize found.\n\r", ch);
	    return;
	  }
	}
    }
...
void display_resets (CHAR_DATA * ch)
{
    ...
    send_to_char
        (" No.  Loads    Description       Location         Vnum   Mx Mn Perc Description"
         "\n\r"
         "==== ======== ============= =================== ======== ===== ==== ==========="
         "\n\r", ch);
    ...
    for (pReset = pRoom->reset_first; pReset; pReset = pReset->next)
    {
        ...
        switch (pReset->command)
        {
            ...
            case 'M':
                ...
                pMob = pMobIndex;
                sprintf (buf,
                         "M[%5ld] %-13.13s in room             R[%5ld] %2ld-%2ld %3d %-15.15s\n\r",
                         pReset->arg1, pMob->short_descr, pReset->arg3,
                         pReset->arg2, pReset->arg4, 
			 pReset->percent, pRoomIndex->name);
                strcat (final, buf);
            ...
            case 'O':
                ...
                sprintf (buf, "O[%5ld] %-13.13s in room             "
                         ...
                         pReset->arg3, pReset->percent, pRoomIndex->name);
                strcat (final, buf);
            ...
            case 'P':
                ...
                sprintf (buf,
                         "O[%5ld] %-13.13s inside              O[%5ld] %2ld-%2ld %3d %-15.15s\n\r",
                         pReset->arg1,
                         pObj->short_descr,
                         pReset->arg3,
                         pReset->arg2, pReset->arg4, pReset->percent,
            ...
            case 'E':
                ...
                {
                    sprintf (buf,
                             "O[%5ld] %-13.13s in the inventory of S[%5ld]      %3d %-15.15s\n\r",
                             pReset->arg1, 
                    ...
                    sprintf (buf,
                             "O[%5ld] %-13.13s %-19.19s M[%5ld]       %3d %-15.15s\n\r",
                             pReset->arg1,
                             pObj->short_descr,
                             (pReset->command == 'G') ?
                             ...
                             pMob->vnum, 
			     pReset->percent, pMob->short_descr);
                strcat (final, buf);
                ...
                || (!str_cmp (arg2, "obj") && is_number (arg3)))
        {
            ...
            if (!str_cmp (arg2, "obj"))
            {
                ...
                if (!str_prefix (arg4, "inside"))
                {
                    ...
                    if ((temp = get_obj_index (is_number (arg5) ? atoi (arg5) : 1))== NULL)
                    {
...
void do_alist (CHAR_DATA * ch, char *argument)
{
    ...
    for (pArea = area_first; pArea; pArea = pArea->next)
    {
        if (!grep || strstr(pArea->name, argument))
        {
          sprintf (buf,
                 "[%3ld] %-*s (%-5ld-%5ld) %-12.12s [%d] [%-10.10s]\n\r",
                 pArea->vnum, 29,
